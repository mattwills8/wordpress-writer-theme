<?php
 $expanded = ( $this->parent->args['open_expanded'] ) ? ' fully-expanded' : '' . ( ! empty( $this->parent->args['class'] ) ? ' ' . esc_attr( $this->parent->args['class'] ) : '' ); $nonce = wp_create_nonce( "redux_ajax_nonce" . $this->parent->args['opt_name'] ); ?>
<div class="redux-container<?php echo esc_attr( $expanded ); ?>">
    ...
    <form method="post" 
        ...
        <?php } ?>
        <?php
 $this->init_settings_fields(); $this->parent->options['last_tab'] = ( isset( $_GET['tab'] ) && ! isset( $this->parent->transients['last_save_mode'] ) ) ? esc_attr( $_GET['tab'] ) : ''; ?>
        <?php ?>
        <input type="hidden" 
               id="last_tab" 
